...
async def async_chain_run(chain, obs, state_name, available_actions, rules):
    try:
        inputs = {
            "observation": obs,
...
def eval(
    ...
    # TODO: record rule path
    # TODO: record rule's selection
    np.random.seed(seed)
    ...
    # llm = FakeListLLM(responses=["selected_action: search[asdfg]\n\n"])
        ...
        while True:
            ...
            for env_i in range(len(envs)):
                if env_status[env_i]==1:
                    ...
                    queries.append((obs, state_name, available_actions, [r.content for r in query_rulelist[env_i]]))
                else:
                    queries.append(None)
...
if __name__ == "__main__":
    ...
    for child in rulesets_path.iterdir():
        if child.is_dir():
            ...
            eval_record_path = ruleset_dir / "eval" / "eval_record_500.json"
        eval(
            envs,
